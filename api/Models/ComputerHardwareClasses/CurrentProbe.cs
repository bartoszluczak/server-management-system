namespace ServerManagementSystem.Models.ComputerHardwareClasses
{
    public class CurrentProbe
    {
        public string Accuracy { get; set; }
        public string Availability { get; set;}
        public string Caption { get; set;}
        public string ConfigManagerErrorCode { get; set;}
        public string ConfigManagerUserConfig { get; set;}
        public string CreationClassName { get; set;}
        public string CurrentReading { get; set;}
        public string Description { get; set;}
        public string DeviceID { get; set;}
        public string ErrorCleared { get; set;}
        public string ErrorDescription { get; set;}
        public string InstallDate { get; set;}
        public string IsLinear { get; set;}
        public string LastErrorCode { get; set;}
        public string LowerThresholdCritical { get; set;}
        public string LowerThresholdFatal { get; set;}
        public string LowerThresholdNonCritical { get; set;}
        public string MaxReadable { get; set;}
        public string MinReadable { get; set;}
        public string Name { get; set;}
        public string NominalReading { get; set;}
        public string NormalMax { get; set;}
        public string NormalMin { get; set;}
        public string PNPDeviceID { get; set;}
        public string PowerManagementCapabilities { get; set;}
        public string PowerManagementSupported { get; set;}
        public string Resolution { get; set;}
        public string Status { get; set;}
        public string StatusInfo { get; set;}
        public string SystemCreationClassName { get; set;}
        public string SystemName { get; set;}
        public string Tolerance { get; set;}
        public string UpperThresholdCritical { get; set;}
        public string UpperThresholdFatal { get; set;}
        public string UpperThresholdNonCritical { get; set;}
    }
}
